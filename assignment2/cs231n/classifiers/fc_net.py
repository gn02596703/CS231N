...
class TwoLayerNet(object):
  ...
  def __init__(self, input_dim=3*32*32, hidden_dim=100, num_classes=10,
    ...
    ############################################################################
    self.params['W1'] = np.random.randn(input_dim, hidden_dim) * weight_scale # * sqrt(2.0/n)
    self.params['b1'] = np.zeros(hidden_dim)
    self.params['W2'] = np.random.randn(hidden_dim, num_classes) * weight_scale # * sqrt(2.0/n)
  ...
  def loss(self, X, y=None):
    ...
    ############################################################################
    hidden_layer, hidden_cache = affine_forward(X, self.params['W1'], self.params['b1'])
    ...
    dHidden, grads['W2'], grads['b2'] = affine_backward(dSoftmax, scores_cache)
    dX, grads['W1'], grads['b1'] = affine_backward(dHidden, hidden_cache)
    # Regularization
...
class FullyConnectedNet(object):
  ...
  def loss(self, X, y=None):
    ...
    scores, cache[self.num_layers -1] = affine_forward(out_pre_layer, 
                                                        self.params[W_name_last], 
                                                        self.params[b_name_last])
    ############################################################################
    #                             END OF YOUR CODE                             #
    ############################################################################
